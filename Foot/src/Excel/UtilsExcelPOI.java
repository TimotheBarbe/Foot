...
public class UtilsExcelPOI {
	...
	public static ArrayList<String> getColumn(int indexColonne,
			Workbook workbook, int sheetNb) {
		ArrayList<String> colonne = new ArrayList<String>();
		Sheet sheet = workbook.getSheetAt(sheetNb);
		...
		try {
			...
			for (int i = 0; i < clubs.size(); i++) {
				for (int j = i + 1; j < clubs.size(); j++) {
					matriceExtraite[i][j] = getDistance(clubs.get(i),
							clubs.get(j), affiliation, matriceDistances);
	...
	public static double getDistance(int clubA, int clubB,
		...
		if (clubA == clubB) {
			...
			if (a > b) {
				distance = Double.parseDouble(matriceDistances[a][b].replace(
						",", "."));
			} else {
				distance = Double.parseDouble(matriceDistances[b][a].replace(
						",", "."));
			}
	...
	public static String getCell(int ligne, int colonne, Workbook workbook) {
		return getColumn(colonne, workbook, 0).get(ligne);
	}
	...
	public static void main(String[] args) {
		...
		try {
			...
			Sheet sheet = workbook.getSheetAt(0);
			// On recupere les numeros d'affiliation des clubs
			ArrayList<String> colonne = getColumn(0, workbook, 0);
